...
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end, bool share)
{
    ...
    do
    {
        ...
        if (*ptep & PTE_V)
        {
            ...
            /* Determine whether we should share the page via copy‑on‑write or
             * make a private copy.  When |share| is true we will map the
             * existing physical page into the child page table, remove its
             * write permission, mark it as copy‑on‑write, and increment the
             * reference count.  When |share| is false we fall back to the
             * traditional behaviour of allocating a new page and copying
             * contents from the parent.
             */
            if (share)
            {
                // share the physical page (optionally with copy‑on‑write semantics)
                struct Page *page = pte2page(*ptep);
                uint32_t perm = (*ptep & PTE_USER);

                /*
                 * Only originally-writable pages should be converted into COW.
                 * If the parent mapping is read-only (e.g. text segment), we
                 * simply share it as-is.  This prevents a process from gaining
                 * write access to pages that were never writable.
                 */
                if (perm & PTE_W)
                {
                    perm &= ~PTE_W;
                    perm |= PTE_COW;
                    // also downgrade the parent's mapping
            ...
            {
                // Traditional behaviour: allocate a new page and copy contents
                uint32_t perm = (*ptep & PTE_USER);
                struct Page *page = pte2page(*ptep);
