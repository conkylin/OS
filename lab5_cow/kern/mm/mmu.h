...
/*
 * PTE_COW - Copy‑on‑Write flag
 *
 * We use one of the software reserved bits in the RISC‑V Sv32 page table
 * entry to mark a mapping as copy‑on‑write. When a mapping is marked
 * copy‑on‑write (COW) the kernel will initially share the physical page
 * between parent and child and remove the write permission from both
 * mappings. On a subsequent write access by either party the page fault
 * handler will allocate a private copy of the page and update the faulting
 * mapping with write permission.  Bit 8 (0x100) is chosen because it falls
 * within the "reserved for software" range and does not conflict with the
