...
exit(int error_code) {
  ...
  8000d4:	51050513          	addi	a0,a0,1296 # 8005e0 <main+0xd6>
...
umain(void) {
  ...
  8000e4:	e406                	sd	ra,8(sp)
    int ret = main();
  8000e6:	424000ef          	jal	ra,80050a <main>
    exit(ret);
...
printnum(void (*putch)(int, void*), void *putdat,
  ...
  80012c:	4d078793          	addi	a5,a5,1232 # 8005f8 <main+0xee>
...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap) {
  ...
  800188:	4a8a8a93          	addi	s5,s5,1192 # 80062c <main+0x122>
        case 'e':
            err = va_arg(ap, int);
            if (err < 0) {
                err = -err;
  ...
  8003e6:	24660613          	addi	a2,a2,582 # 800628 <main+0x11e>
  ...
  8003f8:	22460613          	addi	a2,a2,548 # 800618 <main+0x10e>
  ...
  80040a:	20a40413          	addi	s0,s0,522 # 800610 <main+0x106>
  ...
  800494:	18040413          	addi	s0,s0,384 # 800610 <main+0x106>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  ...
  8004aa:	16ba0a13          	addi	s4,s4,363 # 800611 <main+0x107>
...
memset(void *s, char c, size_t n) {
    ...
    char *p = s;
    while (n -- > 0) {
  8004f8:	ca01                	beqz	a2,800508 <memset+0x10>
  8004fa:	962a                	add	a2,a2,a0
    char *p = s;
  8004fc:	87aa                	mv	a5,a0
        *p ++ = c;
  8004fe:	0785                	addi	a5,a5,1
  800500:	feb78fa3          	sb	a1,-1(a5)
    while (n -- > 0) {
  800504:	fec79de3          	bne	a5,a2,8004fe <memset+0x6>
    }
    return s;
...
int
main(void) {
  80050a:	1101                	addi	sp,sp,-32
    memset(buf, 0xAA, sizeof(buf));
  80050c:	6605                	lui	a2,0x1
  80050e:	0aa00593          	li	a1,170
  800512:	00001517          	auipc	a0,0x1
  800516:	aee50513          	addi	a0,a0,-1298 # 801000 <buf>
main(void) {
  80051a:	ec06                	sd	ra,24(sp)
  80051c:	e822                	sd	s0,16(sp)
    memset(buf, 0xAA, sizeof(buf));
  80051e:	fdbff0ef          	jal	ra,8004f8 <memset>
    int pid = fork();
  800522:	bbdff0ef          	jal	ra,8000de <fork>
    if (pid < 0) {
  800526:	08054063          	bltz	a0,8005a6 <main+0x9c>
  80052a:	87aa                	mv	a5,a0
        cprintf("cowtest: fork failed\n");
        return 1;
    }
    if (pid == 0) {
  80052c:	c549                	beqz	a0,8005b6 <main+0xac>
        // 返回修改后的第一个字节
  ...
  800530:	006c                	addi	a1,sp,12
    int exit_code = -1;
  800532:	c63e                	sw	a5,12(sp)
    int status = waitpid(pid, &exit_code);
  800534:	badff0ef          	jal	ra,8000e0 <waitpid>
    // 检查缓冲区是否被修改
    if ((unsigned char)buf[0] != 0xAA) {
  800538:	00001597          	auipc	a1,0x1
  80053c:	ac85c583          	lbu	a1,-1336(a1) # 801000 <buf>
  800540:	0aa00793          	li	a5,170
    int status = waitpid(pid, &exit_code);
  800544:	842a                	mv	s0,a0
    if ((unsigned char)buf[0] != 0xAA) {
  800546:	02f59463          	bne	a1,a5,80056e <main+0x64>
        cprintf("cowtest failed: parent buffer modified (buf[0]=%x)\n", (unsigned char)buf[0]);
        return 1;
    }
    if (status != 0) {
  80054a:	e529                	bnez	a0,800594 <main+0x8a>
        cprintf("cowtest failed: waitpid returned %d\n", status);
        return 1;
    }
    if (exit_code != 0x55) {
  80054c:	45b2                	lw	a1,12(sp)
  80054e:	05500793          	li	a5,85
  800552:	02f58663          	beq	a1,a5,80057e <main+0x74>
        cprintf("cowtest failed: unexpected child exit code %d\n", exit_code);
  800556:	00000517          	auipc	a0,0x0
  80055a:	43250513          	addi	a0,a0,1074 # 800988 <error_string+0x140>
  ...
  800566:	8522                	mv	a0,s0
  800568:	6442                	ld	s0,16(sp)
  80056a:	6105                	addi	sp,sp,32
  80056c:	8082                	ret
        cprintf("cowtest failed: parent buffer modified (buf[0]=%x)\n", (unsigned char)buf[0]);
  80056e:	00000517          	auipc	a0,0x0
  800572:	3ba50513          	addi	a0,a0,954 # 800928 <error_string+0xe0>
  800576:	acbff0ef          	jal	ra,800040 <cprintf>
        return 1;
  80057a:	4405                	li	s0,1
  80057c:	b7e5                	j	800564 <main+0x5a>
    cprintf("cowtest pass. parent buf[0]=%x, child exit=%d\n",
  80057e:	05500613          	li	a2,85
  800582:	0aa00593          	li	a1,170
  800586:	00000517          	auipc	a0,0x0
  80058a:	43250513          	addi	a0,a0,1074 # 8009b8 <error_string+0x170>
  80058e:	ab3ff0ef          	jal	ra,800040 <cprintf>
    return 0;
  800592:	bfc9                	j	800564 <main+0x5a>
        cprintf("cowtest failed: waitpid returned %d\n", status);
  800594:	85aa                	mv	a1,a0
  800596:	00000517          	auipc	a0,0x0
  80059a:	3ca50513          	addi	a0,a0,970 # 800960 <error_string+0x118>
  80059e:	aa3ff0ef          	jal	ra,800040 <cprintf>
        return 1;
  8005a2:	4405                	li	s0,1
  8005a4:	b7c1                	j	800564 <main+0x5a>
        cprintf("cowtest: fork failed\n");
  8005a6:	00000517          	auipc	a0,0x0
  8005aa:	36a50513          	addi	a0,a0,874 # 800910 <error_string+0xc8>
  8005ae:	a93ff0ef          	jal	ra,800040 <cprintf>
        return 1;
  8005b2:	4405                	li	s0,1
  8005b4:	bf45                	j	800564 <main+0x5a>
  8005b6:	00001697          	auipc	a3,0x1
  8005ba:	a4a68693          	addi	a3,a3,-1462 # 801000 <buf>
            buf[i] = 0x55;
  8005be:	05500593          	li	a1,85
        for (int i = 0; i < sizeof(buf); i++) {
  8005c2:	6605                	lui	a2,0x1
            buf[i] = 0x55;
  8005c4:	00f68733          	add	a4,a3,a5
  8005c8:	00b70023          	sb	a1,0(a4)
        for (int i = 0; i < sizeof(buf); i++) {
  8005cc:	0785                	addi	a5,a5,1
  8005ce:	fec79be3          	bne	a5,a2,8005c4 <main+0xba>
        exit((int)(unsigned char)buf[0]);
  8005d2:	0006c503          	lbu	a0,0(a3)
